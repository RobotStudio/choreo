...
var choreoIOClient = require('../../helpers/choreo.io.js');
...
describe('Pubsub hook', function () {
  ...
  describe('publishAdd()', function () {
      ...
      app.graph({
        ...
        routes: {
          ...
          'POST /dock/:id/addVessel': function (req, res) {
            ...
            app.models.dock.publishAdd(
              req.param('id'), 'vessels', req.param(
                'vessel'));
            return res.send();
          }
        }
    ...
    // Build choreo.io client instance and connect some client socks
    var clientSocks = {
      leonora: undefined,
      lenny: undefined,
      larry: undefined,
    ...
    before(function (done) {
      var io = choreoIOClient(socketIOClient);
      io.choreo.autoConnect = false;
      // (to disable logs from choreo.io client)
      io.choreo.environment = 'production';
      io.choreo.url = 'http://localhost:1535';
      async.each(_.keys(clientSocks), function (key, next) {
        clientSocks[key] = io.choreo.connect();
    ...
    // Use the endpoints defined above to subscribe sockets to a Dock
    // record with id=1.
    before(function (done) {
      async.each(_.keys(clientSocks), function (key, next) {
        clientSocks[key].put('/dock/1/subscribe', function (
          body, jwr) {
    ...
    describe('invoked with the id of the child record', function () {
      ...
      it(
        'should notify all client sockets subscribed to parent record',
        function () {
          _.each(clientSocks, function (socket, key) {
            assert.equal(socket._receivedDockEvents.length,
              1);
            assert.equal(socket._receivedDockEvents[0].verb,
              'addedTo');
            assert.equal(socket._receivedDockEvents[0].id, 1);
            assert.equal(socket._receivedDockEvents[0].addedId,
              47);
          });
        });
    });

    describe('invoked with an object representing the child record',
      function () {
        ...
        it(
          'should notify client sockets subscribed to parent record',
          function () {
            _.each(clientSocks, function (socket, key) {
              assert.equal(socket._receivedDockEvents.length,
                2);
              assert.equal(socket._receivedDockEvents[1].verb,
                'addedTo');
              assert.equal(socket._receivedDockEvents[1].id, 1);
              assert.equal(socket._receivedDockEvents[1].addedId,
                52);
              assert.equal(socket._receivedDockEvents[1].added
                .id, 52);
              assert.equal(socket._receivedDockEvents[1].added
                .name, 'The Silver Goose');
            });
    ...
    describe(
      'invoked with an object representing the child record, but ' +
      'missing the primary key value',
      function () {
        ...
        it(
          'should NOT notify any of the client sockets subscribed to parent ' +
          'record',
          function () {
            _.each(clientSocks, function (socket, key) {
              assert.equal(socket._receivedDockEvents.length,
