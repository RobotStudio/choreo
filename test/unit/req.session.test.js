  ...
  describe('when responding to a virtual request', function () {
    ...
    before(function setupTestRoute() {
      ...
      app.get('/sessionTest', function (req, res) {
        doesSessionExist = !!req.session;
        isSessionAnObject = _.isObject(req.session);
        doesTestPropertyStillExist = req.session.something ===
          'some string';
        res.send();
      });
    ...
    it('should exist', function (done) {
      app.request({
        ...
        if (res.statusCode !== 200) return done(new Error(
          'Expected 200 status code'));
        if (!doesSessionExist) return done(new Error(
          'req.session should exist.'));
        if (!isSessionAnObject) return done(new Error(
    ...
    it('should persist data between requests', function (done) {
      app.request({
        ...
        if (err) return done(err);
        if (clientRes.statusCode !== 200) return done(new Error(
          'Expected 200 status code'));
        if (!doesSessionExist) return done(new Error(
          'req.session should exist.'));
        if (!isSessionAnObject) return done(new Error(
        ...
        app.request({
          ...
          if (err) return done(err);
          if (clientRes.statusCode !== 200) return done(
            new Error('Expected 200 status code'));
          if (!doesSessionExist) return done(new Error(
            'req.session should exist.'));
          if (!isSessionAnObject) return done(new Error(
            'req.session should be an object.'));
          if (!doesTestPropertyStillExist) return done(new Error(
            '`req.session.something` should still exist for subsequent requests.'
          ));
          return done();
        });
      });
