  ...
  return function initializeHooks(hooks, cb) {
    ...
    function prepareHook(id) {
      ...
      if (hooks.userconfig && !hooks.moduleloader) {
        return cb(
          'Invalid configuration:: Cannot use the `userconfig` hook ' +
          'w/o the `moduleloader` hook enabled!');
      }

      // Handle folder-defined modules (default to index.js)
      // Since a hook definition must be a function
      if (_.isObject(hookPrototype) &&
        !_.isArray(hookPrototype) &&
        !_.isFunction(hookPrototype)) {
        hookPrototype = hookPrototype.index;
      }

      if (!_.isFunction(hookPrototype)) {
        choreo.log.error('Malformed hook! (' + id + ')');
        choreo.log.error('Hooks should be a function with one argument ' +
          '(`choreo`)');
        process.exit(1);
      }

      // Instantiate the hook
      var def = hookPrototype(choreo);

      // Mix in an `identity` property to hook definition
      def.identity = id.toDropCase();
    ...
    function applyDefaults(hook) {
      ...
      defaultsDeep(choreo.config, defaults);
    }
    ...
    function loadHook(id, cb) {
      ...
      var timeoutInterval =
        (choreo.config[hooks[id].configKey || id] &&
          choreo.config[hooks[id].configKey || id]._hookTimeout) ||
        choreo.config.hookTimeout || 20000;
      var hookTimeout;
      if (id != 'userhooks') {
        hookTimeout = setTimeout(function tooLong() {
          var hooksTookTooLongErr = 'The hook `' + id +
            '` is taking too long ' +
            'to load.\n' +
            'Make sure it is triggering its `initialize()` callback, ' +
            'or else set `choreo.config.' + (hooks[id].configKey || id) +
            '._hookTimeout to a higher value (currently ' +
            timeoutInterval + ')';
      ...
      hooks[id].load(function (err) {
        ...
        if (err) {
          if (id != 'userhooks') {
            choreo.log.error('A hook (`' + id + '`) failed to load!');
          }
          choreo.emit('hook:' + id + ':error');
          return cb(err);
        }

        choreo.log.verbose(id, 'hook loaded successfully.');
        choreo.emit('hook:' + id + ':loaded');
    ...
    async.series({
        ...
        prepare: function (cb) {
          async.each(_.without(_.keys(hooks),
              'userconfig',
              'moduleloader',
              'userhooks'),
            function (id, cb) {
        ...
        load: function (cb) {
          async.each(_.without(_.keys(hooks),
              ...
              'userhooks'),
            function (id, cb) {
              choreo.log.silly('Loading hook: ' + id);
              loadHook(id, cb);
            }, cb);
        }
      },
